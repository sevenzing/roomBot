...
class CheckTime(Thread):
    def run(self):
        while 1:
            pythoncalendar.checkTime(db, bot)
            time.sleep(60)
...
def sendNextCleaningDay(message: Message):
    ...
    if prev_date is None:
        bot.send_message(chat_id, HAVE_NOT_DAY)
        return

    nextCleaning = pythoncalendar.getNextCleaningDay(prev_date)

    bot.send_message(chat_id,
                     NEXT_DAY % (src.ordinal(nextCleaning.day), pythoncalendar.getMonthName(nextCleaning)),
                     parse_mode='Markdown')
...
def setSchedule(message: Message):
    ...
    bot.send_message(message.chat.id, CALENDAR_SEND, reply_markup=pythoncalendar.create_calendar())
    update(db, chat_id)
...
def callback_query(call):
    ...
    result = pythoncalendar.process_calendar_selection(bot, call)
    if not result is None:
        update(db, chat_id, chosenday=result.__repr__())
